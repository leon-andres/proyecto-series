...
Por lo tanto quiere decir que el primer periodo dominante es $P=\frac{1}{52.22654}\times365=6.988$ días, el segundo es $P=\frac{1}{104.1377}\times365=3.5049$ días y el tercero es $P=\frac{1}{52.0463}\times365=7.0129$
dias. 
...
-   **Hipótesis nula**: No hay estructura no lineal en la serie.
-   **Hipótesis alternativa**: La serie tiene dependencia no lineal, lo que indica que modelos lineales como ARIMA pueden no ser adecuados.
...
-   **Hipótesis nula**: La serie de tiempo sigue un modelo **AR(lineal)**
-   **Hipótesis alternativa**: La serie de tiempo presenta comportamiento nolieanl.
```{r}
#| echo: false
...
Sería una buena práctica en este caso implementar y comparar un modelo SARIMA con alguno más complejo que permita capturar mejor la no linealidad como por ejemplo LSTM.

```{r}
#| echo: false
#| execute: false
# Aquí están todos los objetos de este cuaderno
save.image(file = 'ObjetosDescriptiva.Rdata')
```
